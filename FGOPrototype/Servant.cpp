...
Servant::Servant(std::ifstream& servantDB)
{
	...
	while (std::getline(servantDB, placeHolder))
	{
		...
		case(11):
			...
			break;
		case(12):
			Servant::StarAbsorption = std::stoi(placeHolder);
			break;
		case(13):
			Servant::StarGeneration = std::stoi(placeHolder);
			break;
		case(14):
			Servant::NPGainATK = std::stoi(placeHolder);
			break;
		case(15):
			Servant::NPGainDEF = std::stoi(placeHolder);
			break;
		case(16):
		...
		case(32):
			...
			break;
		case(33):
			Servant::NumOfPassives = std::stoi(placeHolder);
			break;
		case(34):
		...
		case(36):
			...
			break;
		case(37):
			Servant::NPHits = std::stoi(placeHolder);
			break;
		case(38):
	...
	//Refine skill levels because skills are blocked by default until conditions are met

	Servant::SetNPLevel(1);
	Servant::NPGauge = 0;

	//Initialising Card Vector
	int BusterCount = 0;
	int ArtsCount = 0;
	int QuickCount = 0;
	...
	{
		Card Buster(false, "Buster", Servant::BusterHits, BusterCount + 1);
	...
	{
		Card Arts(false, "Arts", Servant::ArtsHits, ArtsCount + 1);
	...
	{
		Card Quick(false, "Quick", Servant::QuickHits, QuickCount + 1);
		Servant::ServantCards.push_back(Quick);
		QuickCount++;
	}

	Card Extra(false, "Extra", Servant::ExtraHits, 1);
	Servant::ServantCards.push_back(Extra);

	Card NP(true, Servant::NPCardType, Servant::NPHits, 1);
	Servant::ServantCards.push_back(NP);

	std::cout << "Object " << Servant::Name << " has been created with allocated stats" << std::endl;
}

//Function to Get Servant Data - Used for debugging (temporarily, at least)
...
//Function to check Servant Cards - Used for debugging (temporarily, at least)
void Servant::CheckCards()
{
	...
	for (int i = 0; i < 7; i++)
	{
		...
		if (card.IsNoblePhantasm() != true && card.GetCardType() != "Extra")
		{
...
//Return Card index from the vector 
Card Servant::ReturnCard(int index)
{
	return Servant::ServantCards[index];
}

//Function to change the NP Level of the servant
//Can be overloaded to increase NP level
